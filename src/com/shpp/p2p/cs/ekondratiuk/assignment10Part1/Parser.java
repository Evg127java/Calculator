...
public class Parser {
    ...
    static List<String> getLexemesList(String line) {
        ...
        Pattern pattern = Pattern.compile("[a-zA-Z]+\\s*|[0-9.]+|[^0-9a-z]\\s*");
        ...
        for (String s : lexemesList) {
            ...
            if (Verifier.isOperator(current)) {
                ...
                if (current.equals(")")) {
                    ...
                    stack.pop();
                    /* Take off the operator from the not empty stack
                    after the open brace if it is the unary one */
                    if (stack.size() > 0 && Verifier.isUnaryOperator(stack.peek())) {
                        output.add(stack.pop());
                    }
                    continue;
                }
                ...
                if (!stack.empty() && getPriority(current) <= getPriority(stack.peek())) {
                    output.add(stack.pop());
                }
                stack.push(current);
            } else {
                ...
                if (!stack.empty() &&
                        Verifier.isOperator(stack.peek()) && getPriority(stack.peek()) > 3) {
                    output.add(stack.pop());
                }
            }
        }
